...
<script>if (screen.width <= 699 && <?php echo $_REQUEST["s"] ?> != true) {document.location = "mobile.php";} </script>
	...
	<script>
		...
		function admin() {
			...
			xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					var pwdInput = prompt("Skriv det hemliga lösenordet!");
		...
		function get_n_of_streck(name, section, log, update) {
			...
			if (!update) {
				...
				var profilebutton = "<td><button class='streckbutton' onclick = profile('" + name + "')>Profil</button></td>";
				var buttons = "";
				if (counter < 200000) {
					for (i = 0; i < ui.length; i++) {
				...
				document.getElementById(name).innerHTML = row + streck + buttons + profilebutton;
				usersCounted += 1;
		...
		function sortTable(n) {
			...
			for (i = 1; i < rows.length; i++) {
				if (n == 2 && rows[i].className != title) {
					...
					row.innerHTML = "<td class=sektionLabel colspan=" + Number(3 + ui.length) + ">" + title + "</td>";
				}
			}
			currentSort = n;
		} 
